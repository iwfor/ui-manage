  ...
  module Audit
    module Checks
      ...
      class IpsDetections < Check
        ...
        # Suricata numbers severity 1 as most severe.
        SEVERE = 1
        ...
        def run
          ...
          by_signature = events.group_by { |e| e['inner_alert_signature'] || e['msg'] || 'unknown' }

          by_signature.each do |signature, matches|
            worst = matches.map { |e| (e['inner_alert_severity'] || 3).to_i }.min

            finding(
              subject:  signature,
              severity: worst <= SEVERE ? :high : :medium,
