...
## Phase 7 — Supporting work  ✅ done

- [x] Renderers — done early, in Phase 6; the `audit` command could not
      render without them
- [x] `Anonymizer` named values: `ssid`, `device_name`, `person`, `email`,
      `label`, plus replacement of any registered name wherever it appears in
      free text
- [x] Completion values for `--check`, `--skip`, `--explain`, `--severity`,
      `--fail-on`, `--format`, read from the registry
- [x] README: audit section, exit codes, `audit.toml` schema, and a check
      catalogue generated by `rake catalog` with a test that fails if it
      drifts
- [x] `--timeout` class option, threaded through to curl

Decisions taken along the way:
- The named-value work turned out to be a correctness fix, not a nicety.
  `--anon` was documented as replacing SSIDs and device names and did not:
  `scrub` only recognises the fixed shapes of IPs and MACs, and a name like
  "Guest" or "AP-Garage" has no shape to recognise. Those fields passed
  through in full. Every view that prints a name now runs it through the
  right helper, and registered names are also replaced inside audit finding
  text.
- The README catalogue is generated and guarded by a test rather than
  hand-maintained, because a table of 39 rows goes stale the first time
  someone adds a check.
- Completion values come from `Audit::Registry` and `Renderer::FORMATS`, so
  adding a check or a format updates the completions with it.

Dropped deliberately:
- Recorded JSON fixtures per endpoint. Without a real controller to record
  from, "fixtures" would be the same invented hashes the tests already use,
  with a layer of file indirection in between — no more faithful, and harder
  to read. The mechanism to record real ones now exists instead:
  `ui-manage <command> --json --anon` writes anonymised, secret-free output,
  which is exactly what a fixture should be. Worth doing against the real
  UDM Pro when convenient.
...
- [ ] Firmware advisory list, with a refresh mechanism.
- [ ] `--baseline` could grow a `--baseline-age` warning, so a stale
      baseline does not quietly suppress findings forever.
